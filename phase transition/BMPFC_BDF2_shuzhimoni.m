clear ;
domain.left   = 0; domain.right  = 128;
domain.bottom = 0; domain.top    = 128;
Nx = 128; Ny = Nx;
...
% x=linspace(0 * Lx+hx ,1*Lx ,Nx);
% y=linspace(0 * Ly+hy ,1*Ly ,Ny);
% 
% [xx,yy] = ndgrid(x,y);
%    u0 = exact(xx,yy,0);
% phi0 = 0.2 *(2*rand (Nx,Ny)-1);
pde = ex_bmpfc_data(para);

    phi1_0 = 0.07+0.001*rand(Nx,Ny);
    phi2_0 = 0.07+0.001*rand(Nx,Ny);
    M0_1 = 0;
    M0_2 = 0;
    M0 = [M0_1,M0_2];
    H_1 = 0.25*(0);
...
dt = 1 ; T= 2000;  t0 = 0;tsave = T; Nt=round (T/dt) ; ns=Nt/50;
     time = struct('T',T,'t0',t0,'dt',dt,'tsave',tsave);
     time1 = struct('T',dt,'t0',t0,'dt',dt,'tsave',tsave);
     [phi1_1, phi2_1,M11, u1, ~] = BMPFC_1st_shuzhifunction(pde,domain,Nx,Ny,time1);
figure(1) ; 
% clf ;
% pcolor(x,y,phi1_0 - phi2_0);shading interp; axis image ; view(0,90)
% colormap jet;
surf(xx,yy,phi1_0 - phi2_0);shading interp; 
axis image ;
ax=gca;
...
t=dt;
% figure(2);
% surf(xx,yy,phi1_0 - phi2_0);shading interp; axis image ; view(0,90);
% colormap jet;
% var1=[2000];
for iter = 2:Nt
% u0=real(u0);
...
% if ~isempty(iff)
%     figure;t
% pcolor(x,y,phi1 - phi2);shading interp; axis image ; view(0,90)
surf(xx,yy,phi1_0 - phi2_0);shading interp;  view(0,90)
axis image ;
ax=gca;
ax.XAxis.Visible='off';
ax.YAxis.Visible='off';
