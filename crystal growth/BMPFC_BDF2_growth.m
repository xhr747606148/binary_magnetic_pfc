...
pde = ex_BMPFC_data_growth(para);
    ...
    phi1_0 = init_growth1_cutoff(xx,yy);
    phi2_0 = init_growth1_cutoff2(xx,yy);
    M0_1 = 0;
    M0_2 = 0;
    M0 = [M0_1,M0_2];
%              H_1 = 1/10*(0);
...
dt = 0.1 ; T= 4000; Nt=round (T/dt) ; ns=Nt/200;tsave=T;
    time = struct('T',T,'t0',t0,'dt',dt,'tsave',tsave);
    time1 = struct('T',dt,'t0',t0,'dt',dt,'tsave',tsave);
    [phi1_1, phi2_1,M11, u1, ~] = BMPFC_1st_growth(pde,domain,Nx,Ny,time1);
% pause (0.01)
    figure(1);
 mesh(xx,yy,phi1_0 - phi2_0,'FaceColor','interp', 'EdgeColor','interp');
    zlim([-1, 1]);
