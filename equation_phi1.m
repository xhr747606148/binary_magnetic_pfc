...
mu1=cos(2*pi*x/32)*cos(2*pi*y/32)*cos(t);
...
norm2 = M_1 .* diff(phi1,x) + M_2 .* diff(phi1,y);
...
g = simplify( diff(phi1,t) + M1.* mu1 - M1 .* 1./(128*128).*int(int(mu1,x,0,128),y,0,128));
% g = simplify( diff(phi1,t) + M1.* mu);
