...
% dt_array = [(0.5)^1 (0.5)^2 (0.5)^3 (0.5)^4 (0.5)^5 ]';
dt_array = [(0.5)^1 (0.5)^2 (0.5)^3 (0.5)^4 (0.5)^5 (0.5)^6 (0.5)^7]';
...
%     k2 = kxx + kyy;
    ...
    [phi1_1, phi2_1,M11, u1, ~] = energy_1st(pde,domain,Nx,Ny,time1);
    nplot = round((T-t0)/dt);
    ...
    E0(1,:) = [0,Energy(phi1_0,phi2_0,M0_1,M0_2,H_1,H_2,para.epsilon,para.alpha,para.beta,para.eta,para.eta1,para.eta2,para.gamma12,para.gamma1,para.gamma2,para.theta,para.theta1,para.theta2,pp,qq,hx,hy,k2,para.w0,para.a1,para.a2)];
    E0(2,:) = [dt,Energy(phi1_1,phi2_1,M11_1,M11_2,H_1,H_2,para.epsilon,para.alpha,para.beta,para.eta,para.eta1,para.eta2,para.gamma12,para.gamma1,para.gamma2,para.theta,para.theta1,para.theta2,pp,qq,hx,hy,k2,para.w0,para.a1,para.a2)];
...
%         H_2 = HH2(yy);
        ...
        u0 = u1;
        u1 = u;
   E0(nt+1,:) = [t,Energy(phi1,phi2,M_1,M_2,H_1,H_2,para.epsilon,para.alpha,para.beta,para.eta,para.eta1,para.eta2,para.gamma12,para.gamma1,para.gamma2,para.theta,para.theta1,para.theta2,pp,qq,hx,hy,k2,para.w0,para.a1,para.a2)];
...
hh=figure;
hold on;
lineType = {'-r', '-b','-c','-g','-m','-y','--k'};
for k = 1:maxIt
energy=E{k};
plot(energy(:,1),energy(:,2),char(lineType(k)),'LineWidth',0.2);

end
h= legend('\delta t = 1/2','\delta t = 1/4','\delta t = 1/8','\delta t = 1/16','\delta t = 1/32','\delta t = 1/64','\delta t = 1/128');
set(h, 'EdgeColor', 'none');
xlabel('Time','Fontsize',18);ylabel('Energy ','Fontsize',18,'interpreter','latex');
% % set(gca,'FontSize',18);
% set(gca,'linewidth',1.1)
% set(h,'interpreter','latex');
box on;
grid off;
% saveas(hh,'Energy(dt=(2 4 8 16 32))eta=-0.1.fig')
