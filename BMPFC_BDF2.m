clear; clc; close all;
% load('exact_BMPFC_BDF2(eta=-100).mat');
...
%         H_2 = HH2(yy);
        ...
        g1_hat = (4 .* phi1_1 - phi1_0 + 4 .* phi2_1 - phi2_0)/(2 .* dt.*para.M1) + rhs11(xx,yy,t,para.M1)./para.M1 + rhs21(xx,yy,t,para.M1)./para.M1;
...
%         ga = g1_hat + (a1.^2 + 0.5 .* a2.^2 + S) .* (dt.*M1) .* gphia(1,1).*hx.*hy/(Lx.*Ly) + rhs1(xx,yy,t,M1,epsilon,gamma)./M1 + rhs2(xx,yy,t,M1,epsilon,gamma)./M1;
        ga = g1_hat - 2.*para.a1.*lap_diff(phi1_bar,k2) - 2.*para.a2.*lap_diff(phi2_bar,k2) + (0.5 .* para.a12.^2 + para.S1) .* (2.*dt.*para.M1) .* gphia(1,1).*hx.*hy/(3 .* (Lx.*Ly))- g1_hat_g + gphiag(1,1).*hx.*hy/(Lx.*Ly);
        phia = inv_A(ga,dt,para.M1,k2,para.a12,para.S1); 
        
        g2_hat = (4 .* phi1_1 - phi1_0 - 4 .* phi2_1 + phi2_0)/(2 .* dt.*para.M1) + rhs11(xx,yy,t,para.M1)./para.M1 - rhs21(xx,yy,t,para.M1)./para.M1;
...
%         gb = g2_hat + (a1.^2 - 0.5 .* a2.^2 + S) .* (dt.*M1) .* gphib(1,1).*hx.*hy/(Lx.*Ly) + rhs1(xx,yy,t,M1,epsilon,gamma)./M1 - rhs2(xx,yy,t,M1,epsilon,gamma)./M1;
        gb = g2_hat - 2.*para.a1.*lap_diff(phi1_bar,k2) + 2.*para.a2.*lap_diff(phi2_bar,k2) + (- 0.5 .* para.a12.^2 + para.S1) .* (2.*dt.*para.M1) .* gphib(1,1).*hx.*hy/(3 .* (Lx.*Ly))- g2_hat_g + gphibg(1,1).*hx.*hy/(Lx.*Ly);
...
%         H_2 = HH2(yy);
        ...
        f1_1 = (4.*M11_1 - M0_1)./(2.* para.M2 .* dt)   + rhs3(xx,yy,t,para.w0,para.alpha,para.beta,para.gamma1,para.gamma2,para.eta1,para.eta2,para.theta,para.M2)./para.M2;
        f1_2 = (4.*M11_2 - M0_2)./(2.* para.M2 .* dt)   + rhs4(xx,yy,t,para.w0,para.alpha,para.beta,para.gamma1,para.gamma2,para.eta1,para.eta2,para.theta,para.M2)./para.M2;
...
hh=figure;
% xlim([0 max(dt_array)]);
loglog(dt_array,error1,'d-');
xlabel('Time step size','Fontsize',18,'Interpreter','latex');
%     ylabel('Error\_max')
ylabel('$L^2$ error','Fontsize',18,'Interpreter','latex');
grid off;
hold on;
loglog(dt_array,error2,'o-');
xlabel('Time step size','Fontsize',18,'Interpreter','latex');
%     ylabel('Error\_max')
ylabel('$L^2$ error','Fontsize',18,'Interpreter','latex');
grid off;
hold on;
loglog(dt_array,error3,'*-');
xlabel('Time step size','Fontsize',18,'Interpreter','latex');
%     ylabel('Error\_max')
ylabel('$L^2$ error','Fontsize',18,'Interpreter','latex');
grid off;
hold on;
% refline(3.3264,-1e-6);
j=min(dt_array):(max(dt_array)-min(dt_array))/100:max(dt_array);
% j1=10.^j;
f=10^(10^(-2))*j.^2;
loglog(j,f,'--');
hold on;
% saveas(hh,'L2_error_order(eta=-100).fig')
axis([10^(-3.5),5*10^(-2.5), 10^(-7), 10^(-2)]);
hLegend =legend("L^2 error-\phi_1","L^2 error-\phi_2","L^2 error-M","Ref-slope 2");
